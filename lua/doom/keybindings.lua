...
if not has_value(g.doom_disabled_modules, 'lsp') and
    (not has_value(g.doom_disabled_plugins, 'compe')) then
...
if not has_value(g.doom_disabled_modules, 'web') and
    (not has_value(g.doom_disabled_plugins, 'restclient')) then
    map('n', '<F7>', ':DotHttp<CR>')
end
...
wk.register({
    ['<leader>'] = {
        p = {
            ...
            s = {
                ':PackerSync<CR>', 'Performs PackerClean and then PackerUpdate'
            },
...
wk.register({
    ['<leader>'] = {
        s = {
            ...
            l = {
                ':Telescope session-lens search_session<CR>', 'Session switcher'
            }
        }
    }
})
...
wk.register({
    ['<leader>'] = {
        d = {
            ...
            r = {
                ':call doom#functions#createReport()<CR>', 'Create crash report'
            }
        }
    }
})
