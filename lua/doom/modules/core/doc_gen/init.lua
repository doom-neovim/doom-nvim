...
---
--- Internal tools for documentation generation within neovim.
---
...
doc_gen.configs = {}
doc_gen.configs["mini_doc"] = function()
...
-- Hidden methods
--
--Due the way packer compiles plugins we can't patch these methods within the
--MiniDoc object
...
-- Parsing --------------------------------------------------------------------
--- Parses lines and returns a block array to be handled by the hooks
---@param lines string[] Lines of document
---@param config table MiniDoc configuration
---@return table<Block> Array of blocks
...
H.toc_insert = function(s)
  ...
  if MiniDoc.current.toc == nil or output_format == "markdown" then return end
...
H.add_section_heading = function(s, heading)
  ...
  if doom.core.doc_gen.settings.output_format == "markdown" then return end
  -- Add heading
...
-- Make helper methods globally accessible via the doc_gen module
doc_gen._H = H
