...
-- -- dui
local components = require("doom.modules.features.ui.dui.results")
...
local function make_results()
  ...
  if DOOM_UI_STATE.query.type == "MAIN_MENU" then
    results = crawl({
      tree = require("doom.modules.features.ui.dui.results").main_menu().entries,
      filter = "list",
    })
  elseif DOOM_UI_STATE.query.type == "SHOW_DOOM_SETTINGS" then
    results = crawl({
      tree = doom.settings,
      leaf = (require("doom.modules.features.ui.dui.results").settings)().entry_template,
      filter = "settings",
    })
  elseif DOOM_UI_STATE.query.type == "LIST_ALL_MODULES" then
    results = crawl({
      tree = require("doom.modules.utils").extend(),
      leaf = (require("doom.modules.features.ui.dui.results").modules)().entry_template,
      ...
      leaf = function(_, k, v)
        ...
        if k == "settings" then
          results = crawl({
            tree = v,
            filter = "settings",
            leaf = require("doom.modules.features.ui.dui.results")[k]().entry_template,
            acc = results,
          })
        elseif k == "binds" then
          results = crawl({
