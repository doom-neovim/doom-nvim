...
local function module_get_component_by_name(buf, mc_name)
  local t_nest_found = {}
  local _, root, qp = conf_ui.run_query_on_buf("lua", "doom_module_binds", buf)
  for id, node, metadata in qp:iter_captures(root, buf, root:start(), root:end_()) do
	...
	if name == mc_name then
      table.insert(t_nest_found, node)
...
-- "recursive" picker that allows you to navigate a table of settings and
-- open up Nui input poput to modify a leaf string setting.
--
-- if string -> go to position
-- if table_keys -> picker(table)
-- if table_of_string -> picker(array_of_strings)
--
-- then later, this picker could be reused to navigate through `module.settings_table`
conf_ui.parse_settings_table = function(buf, node, accumulated,level)
end
...
conf_ui.doom_picker_main_menu = function(c)
  ...
  local doom_main_menu_categories = {
  	"open config",
  	"edit user settings",
  	"browse modules",
  	"all binds (todo..)",
  ...
  require("telescope.pickers").new(opts, {
    ...
    finder = require("telescope.finders").new_table({
      results = doom_main_menu_categories,
    }),
    sorter = require("telescope.config").values.generic_sorter(opts),
    attach_mappings = function(_, map)
...
-- add command for this in the `module_binds` module
conf_ui.doom_picker_settings_global = function(c)

  if c.data == nil then
    local t_nest_table_nodes = module_get_component_by_name(c.buf_ref, "doom_root.settings_table")
...
conf_ui.doom_picker_module_settings = function(c) end
...
conf_ui.doom_picker_module_cmds = function(c) end

--
-- PICKER -> MODULE AUTOCMDS
--

conf_ui.doom_picker_module_autocmds = function(c) end
...
conf_ui.doom_binds_table = function(c)
  ...
  if c.buf_ref == nil then
    local p
    if c.selected_module ~= nil then
	-- TODO: add full module path to object....
